<div class="{{ $root ? 'shadow mb-4' : 'border-right-0 border-bottom-0 border-border-left border-top-0 rounded-0 border-secondary' }} card card-body">
  ...
  <form action="/posts/{{$post->id}}/comments" method="post" class="d-none">  
    @csrf
  ...
  @if (isset($post->comments[$comment->id]))
    @include('partials.commentsList', ['allComments' => $post->comments[$comment->id], 'root' => false ])
  @endif
